...
class BaseketballAdapter (
    ...
    var readdata = sqliteDB?.rawQuery("select * from user_baseketregister", null)

    override fun getCount(): Int {
        return arrayList.size
    }
    interface go {
        fun pass (value:String?)
    }
    interface go1 {
        fun pass1(value:String?)
    }
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        img_update?.setOnClickListener {
            ...
            btn_save?.setOnClickListener {
                ...
                sqliteDB = context.openOrCreateDatabase("baseketregister.db", Context.MODE_APPEND,null)
                sqliteDB?.execSQL(" update user_baseketregister set productname = '"+edt_productname.text.toString()+"' ,  productprice = '"+edt_price.text.toString()+"',date = '"+edt_date.text.toString()+"' where id ="+product.id+" ")
    ...
    fun deleteData(id:Int){
        ...
        builder.setPositiveButton(android.R.string.yes) { dialog, which ->
            sqliteDB?.execSQL("delete from user_baseketregister where  id="+id+" ")
            notifyDataSetChanged()
            dialog.dismiss()
            t1.pass1("Pass1")
        }
