...
class TennisActivity : AppCompatActivity(), TennisAdapter.tenn, TennisAdapter.tenn1 {

    var tennislist: ListView?=null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tennislist = findViewById (R.id.tennislist)
        sqliteDB = this.openOrCreateDatabase("tennisregister.db", MODE_APPEND, null)

        val TABLE_CREATE =
            "create table if not exists user_tennisregister( id integer PRIMARY KEY AUTOINCREMENT,productname varchar(20),productprice varchar(15),date varchar(20))"
    ...
    override fun onResume() {
        ...
        val readdata = sqliteDB?.rawQuery("select * from user_tennisregister", null)
        ...
        val adapter = TennisAdapter(this@TennisActivity,productsdetail,layoutInflater,this,this)
        tennislist?.adapter = adapter
        adapter.notifyDataSetChanged()
    }
    private fun showAlerts() {

        val builder = AlertDialog.Builder(this@TennisActivity).create()
        ...
        btn_save.setOnClickListener {
            ...
            if (productname.length==0) {
                ...
                sqliteDB?.execSQL("insert into user_tennisregister(productname,productprice,date) values('"+ productname+"','"+ price+"','"+date+"')")
                builder.dismiss()
                onResume()
            }
        }
