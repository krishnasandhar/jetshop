...
class BillardsActivity: AppCompatActivity(), BillardsAdapter.gone, BillardsAdapter.gone1 {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_billards)

        billardslist = findViewById(R.id.billardslist)
        sqliteDB = this.openOrCreateDatabase("billardsregister.db", MODE_APPEND, null)

        val TABLE_CREATE =
            "create table if not exists user_billardsregister( id integer PRIMARY KEY AUTOINCREMENT,productname varchar(20),productprice varchar(15),date varchar(20))"
    ...
    override fun onResume() {
        ...
        val adapter = BillardsAdapter(this@BillardsActivity,productsdetail,layoutInflater,this,this)
        billardslist?.adapter = adapter
        adapter.notifyDataSetChanged()
    }
    private fun showAlerts() {

        val builder = AlertDialog.Builder(this@BillardsActivity).create()
        ...
        btn_save.setOnClickListener {
            ...
            if (productname.length==0) {
                ...
                sqliteDB?.execSQL("insert into user_billardsregister(productname,productprice,date) values('"+ productname+"','"+ price+"','"+date+"')")
                builder.dismiss()
                onResume()
            }
        }
    ...
    override fun Move(value: String?) {
        onResume()
    }
    override fun Move1(value: String?) {
        onResume()
    }
}