...
class BoxingActivity : AppCompatActivity(), BoxingAdapter.test, BoxingAdapter.testint {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val TABLE_CREATE =
            "create table if not exists user_boxingproducts( id integer PRIMARY KEY AUTOINCREMENT,productname varchar(20),productprice varchar(15),date varchar(20))"
    ...
    override fun onResume() {
        ...
        val adapter = BoxingAdapter(this@BoxingActivity,productsdetail,layoutInflater,this,this)
        boxinglist?.adapter = adapter
        adapter.notifyDataSetChanged()
    }
    private fun showAlerts() {

        val builder = AlertDialog.Builder(this@BoxingActivity).create()
        ...
        btn_save.setOnClickListener {
            ...
            if (productname.length==0) {
                ...
                sqliteDB?.execSQL("insert into user_boxingproducts(productname,productprice,date) values('"+ productname+"','"+ price+"','"+date+"')")
                builder.dismiss()
                onResume()
            }
        }
