...
class AquagymActivity:AppCompatActivity(),AquagymAdapter.testingInt,AquagymAdapter.testingInt2 {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_aquagym)

        aquagymlist = findViewById (R.id.aquagymlist)
        sqliteDB = this.openOrCreateDatabase("productsregister.db",MODE_APPEND,null)

        val TABLE_CREATE =
            "create table if not exists user_productsregister( id integer PRIMARY KEY AUTOINCREMENT,productname varchar(20),productprice varchar(15),date varchar(20))"
    ...
    override fun onResume() {
        ...
        val adapter = AquagymAdapter(this@AquagymActivity,productsdetail,layoutInflater,this,this)
        aquagymlist?.adapter = adapter
        adapter.notifyDataSetChanged()
    }
    private fun showAlerts() {

        val builder = AlertDialog.Builder(this@AquagymActivity).create()
        ...
        btn_save.setOnClickListener {
            ...
            if (productname.length==0) {
                ...
                sqliteDB?.execSQL("insert into user_productsregister(productname,productprice,date) values('"+ productname+"','"+ price+"','"+date+"')")
                builder.dismiss()
                onResume()
            }
        }
