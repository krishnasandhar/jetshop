...
class GymnasticsAdapter(
    ...
    var readdata = sqliteDB?.rawQuery("select * from user_gymnasticsregister", null)
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        img_update?.setOnClickListener {
            ...
            btn_save?.setOnClickListener {
                ...
                sqliteDB = context.openOrCreateDatabase("gymnasticsregister.db", Context.MODE_APPEND,null)
                sqliteDB?.execSQL(" update user_gymnasticsregister set productname = '"+edt_productname.text.toString()+"' ,  productprice = '"+edt_price.text.toString()+"',date = '"+edt_date.text.toString()+"' where id ="+product.id+" ")
    ...
    private fun deleteData(id: Int) {
        ...
        builder.setPositiveButton(android.R.string.yes) { dialog, which ->
            sqliteDB?.execSQL("delete from user_gymnasticsregister where  id="+id+" ")
            notifyDataSetChanged()
            dialog.dismiss()
            g1.Moving1("Moving1")
        }
