...
class GymnasticsActivity : AppCompatActivity(), GymnasticsAdapter.gymnas, GymnasticsAdapter.gymnas1 {

    var gymnasticlist: ListView?=null
    var sqliteDB: SQLiteDatabase?=null

    @SuppressLint("WrongConstant")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_gymnastics)

        gymnasticlist = findViewById(R.id.gymnasticslist)
        sqliteDB = this.openOrCreateDatabase("gymnasticsregister.db", MODE_APPEND, null)

        val TABLE_CREATE =
            "create table if not exists user_gymnasticsregister( id integer PRIMARY KEY AUTOINCREMENT,productname varchar(20),productprice varchar(15),date varchar(20))"
    ...
    override fun onResume() {
        ...
        val adapter = GymnasticsAdapter(this@GymnasticsActivity,productsdetail,layoutInflater,this,this)
        gymnasticlist?.adapter = adapter
        adapter.notifyDataSetChanged()
    }
    private fun showAlerts() {

        val builder = AlertDialog.Builder(this@GymnasticsActivity).create()
        ...
        btn_save.setOnClickListener {
            ...
            if (productname.length==0) {
                ...
                sqliteDB?.execSQL("insert into user_gymnasticsregister(productname,productprice,date) values('"+ productname+"','"+ price+"','"+date+"')")
                builder.dismiss()
                onResume()
            }
        }
