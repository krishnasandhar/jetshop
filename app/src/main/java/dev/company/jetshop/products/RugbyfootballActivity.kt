...
class RugbyfootballActivity: AppCompatActivity(), RugbyfootballAdapter.rugby, RugbyfootballAdapter.rugby1 {

    var rugbyfootballlist: ListView?=null
    var sqliteDB: SQLiteDatabase?=null

    @SuppressLint("WrongConstant")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_rugbyfootball)

        rugbyfootballlist = findViewById(R.id.rugbyfootballlist)
        sqliteDB = this.openOrCreateDatabase("rugbyfootballregister.db", MODE_APPEND, null)

        val TABLE_CREATE =
            "create table if not exists user_rugbyfootballregister( id integer PRIMARY KEY AUTOINCREMENT,productname varchar(20),productprice varchar(15),date varchar(20))"
    ...
    override fun onResume() {
        ...
        val readdata = sqliteDB?.rawQuery("select * from user_rugbyfootballregister", null)
        ...
        val adapter = RugbyfootballAdapter(this@RugbyfootballActivity,productsdetail,layoutInflater,this,this)
        rugbyfootballlist?.adapter = adapter
        adapter.notifyDataSetChanged()
    }
    private fun showAlerts() {

        val builder = AlertDialog.Builder(this@RugbyfootballActivity).create()
        ...
        btn_save.setOnClickListener {
            ...
            if (productname.length==0) {
                ...
                sqliteDB?.execSQL("insert into user_rugbyfootballregister(productname,productprice,date) values('"+ productname+"','"+ price+"','"+date+"')")
                builder.dismiss()
                onResume()
            }
        }
