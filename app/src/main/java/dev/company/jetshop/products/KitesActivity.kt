...
class KitesActivity : AppCompatActivity(), KitesAdapter.kite, KitesAdapter.kite1 {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_kites)

        kiteslist = findViewById(R.id.kiteslist)
        sqliteDB = this.openOrCreateDatabase("kitesregister.db", MODE_APPEND, null)

        val TABLE_CREATE =
            "create table if not exists user_kitesregister( id integer PRIMARY KEY AUTOINCREMENT,productname varchar(20),productprice varchar(15),date varchar(20))"
    ...
    override fun onResume() {
        ...
        val adapter = KitesAdapter(this@KitesActivity,productsdetail,layoutInflater,this,this)
        kiteslist?.adapter = adapter
        adapter.notifyDataSetChanged()
    }
    private fun showAlerts() {

        val builder = AlertDialog.Builder(this@KitesActivity).create()
        ...
        btn_save.setOnClickListener {
            ...
            if (productname.length==0) {
                ...
                sqliteDB?.execSQL("insert into user_kitesregister(productname,productprice,date) values('"+ productname+"','"+ price+"','"+date+"')")
                builder.dismiss()
                onResume()
            }
        }
