...
class CricketAdapter(
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        img_update?.setOnClickListener {
            ...
            btn_save?.setOnClickListener {
                ...
                sqliteDB = context.openOrCreateDatabase("cricketregister.db", Context.MODE_APPEND,null)
                sqliteDB?.execSQL(" update user_cricketregister set productname = '"+edt_productname.text.toString()+"' ,  productprice = '"+edt_price.text.toString()+"',date = '"+edt_date.text.toString()+"' where id ="+product.id+" ")
    ...
    fun deleteData(id:Int){
        ...
        builder.setPositiveButton(android.R.string.yes) { dialog, which ->
            sqliteDB?.execSQL("delete from user_cricketregister where  id="+id+" ")
            notifyDataSetChanged()
            dialog.dismiss()
            c1.Moving2("Moving2")
        }
