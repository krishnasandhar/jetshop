...
class RunningActivity : AppCompatActivity(), RunningAdapter.run, RunningAdapter.run1 {

    var runninglist: ListView?=null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        runninglist = findViewById(R.id.runninglist)
        sqliteDB = this.openOrCreateDatabase("runningregister.db", MODE_APPEND, null)

        val TABLE_CREATE =
            "create table if not exists user_runningregister( id integer PRIMARY KEY AUTOINCREMENT,productname varchar(20),productprice varchar(15),date varchar(20))"
    ...
    override fun onResume() {
        ...
        val readdata = sqliteDB?.rawQuery("select * from user_runningregister", null)
        ...
        val adapter = RunningAdapter(this@RunningActivity,productsdetail,layoutInflater,this,this)
        runninglist?.adapter = adapter
        adapter.notifyDataSetChanged()
    }
    private fun showAlerts() {

        val builder = AlertDialog.Builder(this@RunningActivity).create()
        ...
        btn_save.setOnClickListener {
            ...
            if (productname.length==0) {
                ...
                sqliteDB?.execSQL("insert into user_runningregister(productname,productprice,date) values('"+ productname+"','"+ price+"','"+date+"')")
                builder.dismiss()
                onResume()
            }
        }
