...
class SkatingActivity : AppCompatActivity(), SkatingAdapter.skat, SkatingAdapter.skat1 {

    var skatinglist: ListView?=null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        skatinglist = findViewById(R.id.skatinglist)
        sqliteDB = this.openOrCreateDatabase("skatingregister.db", MODE_APPEND, null)

        val TABLE_CREATE =
            "create table if not exists user_skatingregister( id integer PRIMARY KEY AUTOINCREMENT,productname varchar(20),productprice varchar(15),date varchar(20))"
    ...
    override fun onResume() {
        ...
        val readdata = sqliteDB?.rawQuery("select * from user_skatingregister", null)
        ...
        val adapter = SkatingAdapter(this@SkatingActivity,productsdetail,layoutInflater,this,this)
        skatinglist?.adapter = adapter
        adapter.notifyDataSetChanged()
    }
    private fun showAlerts() {

        val builder = AlertDialog.Builder(this@SkatingActivity).create()
        ...
        btn_save.setOnClickListener {
            ...
            if (productname.length==0) {
                ...
                sqliteDB?.execSQL("insert into user_skatingregister(productname,productprice,date) values('"+ productname+"','"+ price+"','"+date+"')")
                builder.dismiss()
                onResume()
            }
        }
