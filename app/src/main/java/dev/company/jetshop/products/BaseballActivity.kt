...
class BaseballActivity : AppCompatActivity(), BaseballAdapter.move, BaseballAdapter.move1 {

    var baseballist: ListView?=null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        baseballist = findViewById(R.id.baseballlist)
        sqliteDB = this.openOrCreateDatabase("baseballregister.db", MODE_APPEND,null)

        val TABLE_CREATE =
            "create table if not exists user_baseballregister( id integer PRIMARY KEY AUTOINCREMENT,productname varchar(20),productprice varchar(15),date varchar(20))"
    ...
    override fun onResume() {
        ...
        val adapter = BaseballAdapter(this@BaseballActivity,productsdetail,layoutInflater,this,this)
        baseballist?.adapter = adapter
        adapter.notifyDataSetChanged()
    }
    private fun showAlerts() {

        val builder = AlertDialog.Builder(this@BaseballActivity).create()
        ...
        btn_save.setOnClickListener {
            ...
            if (productname.length==0) {
                ...
                sqliteDB?.execSQL("insert into user_baseballregister(productname,productprice,date) values('"+ productname+"','"+ price+"','"+date+"')")
                builder.dismiss()
                onResume()
            }
        }
