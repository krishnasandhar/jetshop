...
class ArcheryAdapter(
    ...
    override fun getView (position: Int, convertView: View?, parent: ViewGroup?): View? {
        ...
        img_update?.setOnClickListener {
            ...
            btn_save?.setOnClickListener {
                ...
                sqliteDB = context.openOrCreateDatabase("archeryproducts.db", Context.MODE_APPEND,null)
                sqliteDB?.execSQL(" update user_archeryproducts set productname = '"+edt_productname.text.toString()+"' ,  productprice = '"+edt_price.text.toString()+"',date = '"+edt_date.text.toString()+"' where id ="+product.id+" ")
    ...
    fun deleteData(id:Int){
        ...
        builder.setPositiveButton(android.R.string.yes) { dialog, which ->
            sqliteDB?.execSQL("delete from user_archeryproducts where  id="+id+" ")
            notifyDataSetChanged()
            dialog.dismiss()
            t2.Moving2("Moving2")
        }
